...
function queryBestMove(engineId, moves, movetime) {
    ...
    if (!enginePath) {
        return Promise.reject(new Error(`${engineId} is unavailable on this backend`));
    }
...
wss.on('connection', (ws) => {
    ...
    ws.on('message', (message) => {
        ...
        if (cmd.startsWith('__DUEL_MOVE__ ')) {
            const [, engineId = '', movetimeRaw = '250', ...moves] = cmd.trim().split(/\s+/);
            queryBestMove(engineId, moves, Number(movetimeRaw) || 250)
                ...
                .catch((error) => {
                    if (ws.readyState === 1) {
                        const prefix = /unavailable/i.test(error.message) ? 'duelunavailable' : 'duelerror';
