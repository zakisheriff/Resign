...
const BOT_PRESETS = [
  {
    ...
    title: "House Engine",
    elo: 6767,
    blurb: "The homegrown engine. Fast, punchy, and always ready to scrap.",
    badge: "R",
    portrait: "RS",
    moveTimeMs: 1000,
    ponderDepth: 12,
    gradient: "linear-gradient(135deg, #7f8f52, #39421f)",
  },
];
...
const BOT_CATEGORIES = [
  {
    id: "adaptive",
    name: "Engines",
    subtitle: "Flexible training bots",
    accent: "#8bc34a",
    preview: "AI",
  },
];
...
const CLASS_LABELS: Record<
  MoveClassification,
  { symbol: React.ReactNode; label: string }
> = {
...
export default function ResignGUI() {
  const STOCKFISH_AVATAR_SRC =
    "https://pbs.twimg.com/media/GXHVoRNXcAAa5fH.jpg";
  const USER_AVATAR_SRC = "/user-icon.svg";
  ...
  // Review mode
  const [panelTab, setPanelTab] = useState<"new" | "review" | "settings">(
    "new",
  );
  ...
  const [selectedBotCategoryId, setSelectedBotCategoryId] =
    useState("adaptive");
  ...
  const selectedBotCategory = useMemo(
    () =>
      botsByCategory.find(
        (category) => category.id === selectedBotCategoryId,
      ) ?? botsByCategory[0],
  ...
  >(() => {
    ...
    for (const [key, filename] of Object.entries(mapping)) {
      result[key] = (props?: any) => (
        ...
        >
          <image
            href={`https://images.chesscomfiles.com/chess-themes/pieces/${pieceSet}/150/${filename}.png`}
            width="150"
            height="150"
          />
        </svg>
  ...
  const analyzePosition = useCallback(
    (fenStr: string, forEngineMove: boolean) => {
      if (!forEngineMove) {
  ...
  const isLegalEngineMove = useCallback((uciMove: string) => {
    ...
    return legalMoves.some(
      (move) => move.to === to && (move.promotion ?? undefined) === promotion,
    );
  }, []);
    ...
    setMoveHistory((prev) => {
      ...
      const nextHistory = prev.map((move, idx) => {
        if (move.fen === fenKey) {
          ...
          if (
            move.evalAfter !== evalAfter ||
            move.evalBefore !== evalBefore ||
            move.cpLoss !== cpLoss ||
        ...
        if (idx > 0 && prev[idx - 1].fen === fenKey) {
          ...
          if (
            nextMove.evalBefore !== evalBefore ||
            nextMove.cpLoss !== cpLoss ||
            nextMove.classification !== classification
          ) {
            changed = true;
            return {
              ...nextMove,
  ...
  useEffect(() => {
    ...
    const wsUrl =
      ...
      (isLocalHost
        ? "ws://localhost:3001"
        : "wss://zakisheriff-resign-backend.hf.space");
          ...
          if (best && best !== "0000" && best !== "(none)") {
            if (isEngineTurnRef.current) {
              if (!isLegalEngineMove(best)) {
                console.log(
                  "Ignored invalid or stale engine bestmove for current board:",
                  best,
                  gameRef.current.fen(),
                );
                continue;
              ...
              try {
                ...
                const evalBefore =
                  evalMap.current[prevFen] ?? lastEvalRef.current;
  ...
  function checkGameEnd() {
    ...
    if (g.isCheckmate()) {
      ...
      if (gameMode === "engine") {
        setStatusText(
          g.turn() === playerColor
            ? "Your king is in check. Move the king or defend it."
            : "Resign is in check.",
        );
      } else {
        setStatusText(
          g.turn() === "w"
            ? "White king is in check. Move the king or defend it."
            : "Black king is in check. Move the king or defend it.",
        );
      }
    }
  }
  ...
  function resolveCastleTargetSquare(
    from: string,
    to: string,
    legalMoves: Move[],
  ) {
    ...
    const castleTarget =
      to.charCodeAt(0) > from.charCodeAt(0) ? `g${from[1]}` : `c${from[1]}`;

    const isLegalCastle = legalMoves.some(
      (move) =>
        move.to === castleTarget &&
  ...
  function recordPlayerMove(move: Move) {
    ...
    if (!gameRef.current.isGameOver()) {
      if (gameMode === "engine") {
        ...
        setStatusText(
          gameRef.current.turn() === "w" ? "White's turn" : "Black's turn",
        );
      }
    }
  }
  ...
  function buildUciHistory() {
    return moveHistoryRef.current.map(
  ...
  function requestNextDuelMove() {
    ...
    setStatusText(
      `${engineId === "resign" ? "Resign" : "Stockfish"} is thinking...`,
    );
    ws.current.send(
      `__DUEL_MOVE__ ${engineId} 250 ${buildUciHistory().join(" ")}`.trim(),
    );
  }
  ...
  const isPlayerPromotionMove = useCallback(
  ...
  const handleCanDragPiece = useCallback(
    ({
      ...
      if (!square || !gameStartedRef.current || isPaused || pendingPromotion)
        return false;
      ...
      const isPlayerTurn =
        currentGameMode === "engine" ? turn === currentPlayerColor : true;
    ...
    ({ piece, square }: { piece: any; square: string }) => {
      ...
      if (currentGameMode === "engine" && !isPlayerTurn) {
        ...
        if (selectedSquare) {
          ...
          const resolvedTargetSquare = resolveCastleTargetSquare(
            selectedSquare,
            square,
            pseudoMoves,
          );
          const matchedMove = pseudoMoves.find(
            (m) => m.to === resolvedTargetSquare,
          );
      ...
      if (clickedPiece && clickedPiece.color === gameRef.current.turn()) {
        setSelectedSquare(square);
        if (
          gameRef.current.moves({ square: square as Square, verbose: true })
            .length === 0
        ) {
          setLegalMoveSquares({});
          if (gameRef.current.isCheck()) {
  ...
  const handlePieceDrop = useCallback(
    ({
      ...
      setDraggedSquare(null);
      if (!gameStartedRef.current || !targetSquare || pendingPromotion)
        return false;
      ...
      if (currentGameMode === "engine" && !isPlayerTurn) {
        ...
        if (isPseudoLegal) {
          const isPromotion = pseudoMoves.some(
            (m) => m.to === resolvedTargetSquare && m.flags.includes("p"),
          );
          queuePreMove({
            from: sourceSquare,
            to: resolvedTargetSquare,
  ...
  const startGame = useCallback(
    (asColor: "w" | "b", mode: GameMode = "engine") => {
      resetEngine();
      ...
      setGameMode(mode);
      setBoardOrientation(
        mode === "duel" ? "white" : asColor === "w" ? "white" : "black",
      );
      setPreMoves([]);
  ...
  const moveStats = useMemo(() => {
    ...
    const evaluatedMoves = playerMoves.filter(
      (m) => typeof m.cpLoss === "number" && !isNaN(m.cpLoss),
    );
    let totalCpLoss = 0;
  ...
  const engineStats = useMemo(() => {
    ...
    const evaluatedMoves = engineMoves.filter(
      (m) => typeof m.cpLoss === "number" && !isNaN(m.cpLoss),
    );
    let totalCpLoss = 0;
  ...
  const renderCustomSquare = useCallback(
    (props: { piece: any; square: string; children?: React.ReactNode }) => {
  ...
  const renderPlayerAvatar = useCallback(
    (kind: "resign" | "stockfish" | "user" | "bot") => {
      if (kind === "resign") {
        return (
      ...
      if (kind === "user") {
        return (
          <img
            src={USER_AVATAR_SRC}
            className="brand-logo-image brand-logo-image--pad"
            alt="User icon"
          />
        );
      }

      if (isLogoBot(selectedBot.id)) {
        return (
          <img
            src="/Logo.png"
            className="brand-logo-image"
            alt={`${selectedBot.name} logo`}
          />
        );
      }
    ...
    [STOCKFISH_AVATAR_SRC, USER_AVATAR_SRC, selectedBot],
  );
  ...
  return (
    <main className="chess-layout">
      <section className="sr-only" aria-label="About Resign Chess">
        <h1>
          Resign Chess lets you play chess online against Resign and custom
          bots.
        </h1>
        <p>
          Challenge engine-powered bots, pick blitz or rapid time controls,
          review moves, choose promotion pieces, and play pass-and-play chess
      ...
      <div className="board-area">
        ...
        <div className="player-info">
          {boardOrientation === "white" ? (
            ...
            <>
              <div className="player-profile">
                <div className="player-avatar">
                  {gameMode === "duel"
                    ? renderPlayerAvatar("stockfish")
                    : gameMode === "engine"
                      ? renderPlayerAvatar("bot")
                      : renderPlayerAvatar("user")}
                </div>
                <span>
                  {gameMode === "duel"
                    ? "Stockfish "
              ...
              <div
                className={`player-clock ${isPlaying && gameRef.current.turn() === "b" ? "active-clock" : ""} ${blackTime <= 0 ? "timeout" : ""}`}
              >
            ...
            <>
              ...
              <div
                className={`player-clock ${isPlaying && gameRef.current.turn() === "w" ? "active-clock" : ""} ${whiteTime <= 0 ? "timeout" : ""}`}
              >
        ...
        <div className="board-wrapper">
          <div
            ...
            style={
              {
                ["--eval-percent" as "--eval-percent"]: `${evalPercent}%`,
              } as React.CSSProperties
            }
          >
            <div className="eval-bar-fill" />
            {evalScore <= 0 && (
              <div className="eval-bar-label top-label">{evalLabel}</div>
            )}
            {evalScore > 0 && (
              <div className="eval-bar-label bottom-label">{evalLabel}</div>
            )}
          </div>
          <div className="board-container">
            <Chessboard
              options={{
                ...
                darkSquareStyle: {
                  backgroundColor: BOARD_THEMES[boardThemeIdx].dark,
                },
                lightSquareStyle: {
                  backgroundColor: BOARD_THEMES[boardThemeIdx].light,
                },
      ...
      <div className="panel">
        <div className="panel-tabs">
          <div
            className={`tab ${panelTab === "new" ? "active" : ""}`}
            onClick={() => {
              setPanelTab("new");
              if (gameStarted) {
                setFen(gameRef.current.fen());
          ...
          <div
            className={`tab ${panelTab === "settings" ? "active" : ""}`}
          ...
          <div className="panel-content">
            {!gameStarted && (
              <>
                ...
                <div className="game-mode-selector">
                  <button
                    className={`mode-tab ${gameMode === "engine" ? "active" : ""}`}
                  ...
                  <button
                    className={`mode-tab ${gameMode === "friend" ? "active" : ""}`}
                  ...
                  <button
                    className={`mode-tab ${gameMode === "duel" ? "active" : ""}`}
                  ...
                  {showTimeDropdown && (
                    <div className="time-dropdown">
                      {TIME_CONTROLS.map((t, i) => (
                        <button
                          key={i}
                          className={`time-option ${i === timeControlIdx ? "selected" : ""}`}
                          onClick={() => {
                            setTimeControlIdx(i);
              ...
              >
                {gameMode === "duel" ? (
                  ...
                  >
                    {isPaused ? (
                      <>
                        <PlayCircle size={16} /> Resume Duel
                      </>
                    ) : (
                      <>
                        <Pause size={16} /> Pause Duel
                      </>
                    )}
                  </button>
                ) : (
                  <div style={{ display: "flex", gap: "8px" }}>
                    <button
                      ...
                      onClick={undoMove}
                      disabled={
                        moveHistory.length === 0 || engineThinking.current
                      }
                    >
                      <Undo2 size={16} />
                    </button>
                    <button
                      ...
                      onClick={redoMove}
                      disabled={
                        undoneMoves.length === 0 || engineThinking.current
                      }
                    >
                      <Redo2 size={16} />
                    </button>
                ...
                <button
                  ...
                  onClick={() => {
                    if (
                      window.confirm("Are you sure you want to stop the game?")
                    ) {
                      stopTimer();
                      setGameStarted(false);
                      setIsPaused(false);
                ...
                {gameMode === "engine" ? (
                  <>
                    <button
                      ...
                      onClick={() => startGame("w", "engine")}
                    >
                      Play as White
                    </button>
                    <button
                      className="btn-secondary"
                      onClick={() => startGame("b", "engine")}
                    >
                      <Crown size={16} color="#f3f3f3" fill="#111111" /> Play as
                      Black
                    </button>
                  </>
                ) : gameMode === "duel" ? (
                  <button
                    className="btn-primary btn-duel-launch"
                    onClick={() => startGame("w", "duel")}
                  >
                    <span className="btn-duel-launch-icon">
                      <Swords size={20} />
                    </span>
            ...
            <button
              ...
              onClick={() => {
                ...
                duelActiveRef.current = false;
                duelBusyRef.current = false;
                setStatusText('Click "Start Game" to begin');
                engineThinking.current = false;
              }}
            >
            ...
            <div className="engine-output">
              {engineLines.length === 0
                ? "Engine output..."
                : engineLines.map((line, i) => <div key={i}>{line}</div>)}
            </div>
              ...
              <div className="move-list" style={{ maxHeight: 160 }}>
                {moveHistory.map((m, i) => (
                  <div key={i} className="move-row">
                    ...
                    <span className={`move-class cls-${m.classification}`}>
                      {CLASS_LABELS[m.classification].symbol}
                    </span>
                  </div>
                ))}
              </div>
          ...
          <div className="panel-content">
            <div className="nav-buttons">
              ...
              <button
                className="nav-btn"
                onClick={() => goToMove(reviewIndex + 1)}
                disabled={reviewIndex >= moveHistory.length - 1}
              >
            ...
            <div className="move-list">
              {moveHistory.map((m, i) => (
                <div
                  key={i}
                  className={`move-row ${i === reviewIndex ? "active-move" : ""}`}
                  onClick={() => goToMove(i)}
                >
                  {m.color === "w" && (
                    <span className="move-number">{m.moveNumber}.</span>
                  ...
                  <span className="move-san" style={{ cursor: "pointer" }}>
                    {m.san}
                  </span>
                  <span className={`move-class cls-${m.classification}`}>
                    {CLASS_LABELS[m.classification].symbol}
            ...
            {reviewIndex !== -1 && moveHistory[reviewIndex] && (
              <div className="coach-comment-box">
                <div className="coach-header">
                  <span
                    className={`move-class-badge cls-${moveHistory[reviewIndex].classification}`}
                  >
                    {
                      CLASS_LABELS[moveHistory[reviewIndex].classification]
                        .symbol
                    }{" "}
                    {
                      CLASS_LABELS[moveHistory[reviewIndex].classification]
                ...
                <div className="coach-text">
                  <strong>{moveHistory[reviewIndex].san}</strong>:{" "}
                  {getMoveExplanation(moveHistory[reviewIndex])}
                </div>
              </div>
            )}
            ...
            <div style={{ display: "flex", gap: "8px", marginTop: "8px" }}>
              ...
              <button
                ...
                onClick={() => {
                  setPanelTab("new");
                  if (gameStarted) setFen(gameRef.current.fen());
                  setReviewIndex(-1);
                }}
              >
            ...
            <div className="settings-section">
              ...
              <div className="theme-grid">
                {BOARD_THEMES.map((theme, i) => (
                  <div
                    key={i}
                    className={`theme-swatch ${i === boardThemeIdx ? "active" : ""}`}
                    onClick={() => changeBoardTheme(i)}
                  >
            ...
            <div className="settings-section">
              ...
              <div className="piece-set-scroll">
                {PIECE_SETS.map((set) => (
                  <div
                    key={set.id}
                    className={`piece-set-option ${set.id === pieceSet ? "active" : ""}`}
                    onClick={() => changePieceSet(set.id)}
                  >
                    <img
                      src={`https://images.chesscomfiles.com/chess-themes/pieces/${set.id}/150/wk.png`}
                      alt={set.name}
                    />
          ...
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="accuracy-row">
              <div className="accuracy-item">
                <div className="accuracy-pct">
                  {moveStats.accuracy.toFixed(1)}%
                </div>
                <div className="accuracy-label">Your Accuracy</div>
              </div>
              <div className="accuracy-item">
                <div className="accuracy-pct" style={{ color: "#e58c2a" }}>
                  {engineStats.accuracy.toFixed(1)}%
                </div>
                ...
                {(
                  ...
                  <tr className="review-stats-row" key={cls}>
                    <td className="review-stats-cell label-cell">
                      <span className="review-stats-symbol">
                        {CLASS_LABELS[cls].symbol}
                      </span>
                      <span>{CLASS_LABELS[cls].label}</span>
                    </td>
                    <td
                      className={`review-stats-cell review-stats-count cls-${cls}`}
                    >
