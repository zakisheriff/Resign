...
static void half_kp_append_active_indices(const NNUE_Position *pos, const int c,
    IndexList *active)
{
...
static void half_kp_append_changed_indices(const NNUE_Position *pos, const int c,
...
static void append_active_indices(const NNUE_Position *pos, IndexList active[2])
{
  for (unsigned c = 0; c < 2; c++)
    half_kp_append_active_indices(pos, c, &active[c]);
}

static void append_changed_indices(const NNUE_Position *pos, IndexList removed[2],
...
INLINE void refresh_accumulator(NNUE_Position *pos)
{
...
INLINE bool update_accumulator(NNUE_Position *pos)
{
  Accumulator *accumulator = &(pos->nnue[0]->accumulator);
...
INLINE void transform(NNUE_Position *pos, clipped_t *output, mask_t *outMask)
{
...
// Evaluation function
int nnue_evaluate_pos(NNUE_Position *pos)
{
  int32_t out_value;
